...
public class AttendanceRepositoryAsync: GenericRepositoryAsync<Attendance>, IAttendanceRepositoryAsync
{
    ...
    public async Task<Attendance?> GetAttendanceByIdAndTenantIdAsync(int attendanceId, int tenantId)
        => await _dbContext.Set<Attendance>()
                           .Include(a => a.Tenant)
                           .SingleOrDefaultAsync(a => a.AttendanceId == attendanceId 
                                                      && a.TenantId == tenantId);

    public async Task<IEnumerable<Attendance>>
        GetAttendancesBetweenDatesByTenantIdAsync(int tenantId,
                                                  DateOnly startDate,
                                                  DateOnly endDate)
        => await _dbContext.Set<Attendance>()
    ...
    public async Task<IEnumerable<Attendance>>
        GetAttendancesForLastYearAndCurrentYearByTenantIdAsync(int tenantId,
                                                               DateOnly startDate,
                                                               DateOnly endDate)
        => await _dbContext.Set<Attendance>()
                           .Where(f => f.TenantId == tenantId
